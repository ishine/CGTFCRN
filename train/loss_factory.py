...
class loss_hybrid(nn.Module):
    ...
    def forward(self, pred_stft, true_stft):
        ...
        mag_loss = torch.mean((torch.relu(true_mag**(0.3) - pred_mag**(0.3)))**2)
        ...
        sisnr =  - torch.log10(torch.sum(torch.square(y_true),dim=-1,keepdim=True) / torch.sum(torch.square(y_true - y_pred),dim=-1,keepdim=True) + 1e-8).mean()
