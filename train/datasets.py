...
class MyDataset(data.Dataset):
    def __init__(self, train_folder, shuffle, num_tot, wav_len=0, n_fft=512, hop_length=256, win_length=512):
        ...
        path = Path(os.path.join(train_folder, 'noisy'))
        self.file_name = [p.name for p in path.iterdir() if p.is_file()]
    ...
    def __getitem__(self, idx):
        noisy, fs = sf.read(os.path.join(self.train_folder, 'noisy', self.file_name[idx]), dtype="float32")
        clean, fs = sf.read(os.path.join(self.train_folder, 'clean', self.file_name[idx]), dtype="float32")
